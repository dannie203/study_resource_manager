    ...
    <article className="bg-white rounded-2xl p-5 flex shadow-md border border-gray-100 hover:shadow-lg transition-all min-h-[110px]">
      <div className="flex flex-col flex-1 gap-1">
        ...
        <div className="flex items-center gap-2 mb-1">
          {resource.status === 'PENDING' && <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded font-bold">Chờ duyệt</span>}
          {resource.status === 'APPROVED' && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded font-bold">Đã duyệt</span>}
          {resource.status === 'REJECTED' && <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded font-bold">Từ chối</span>}
        </div>
        <p className="text-xs text-gray-500">Tên file: <span className="font-mono text-gray-700">{resource.originalName}</span></p>
        <div className="text-xs text-gray-400 font-mono select-text">Ngày tải lên: {new Date(resource.createdAt).toLocaleDateString()}</div>
        <div className="mt-auto flex items-center justify-between pt-2">
          ...
          <div className="flex gap-2">
            <button aria-label="More info" className="text-green-700 hover:text-green-900 text-xs flex items-center gap-1">
